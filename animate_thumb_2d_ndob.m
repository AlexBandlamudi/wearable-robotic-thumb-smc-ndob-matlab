function animate_thumb_2d_ndob(t, Q, Qref, TAU_D_Nm, TAU_DH_Nm, segs, labels, label_clrs, out_base, title_str)
% 2-D animation of the 3-DOF robotic thumb with NDOB overlay.
% Same layout as animate_thumb_2d but overlays the NDOB estimate TAU_DH_Nm on the disturbance panel.
    ...
    if nargin < 5 || isempty(TAU_DH_Nm)
        TAU_DH_Nm = zeros(size(TAU_D_Nm));
    end
    if nargin < 6 || isempty(segs)
        segs = [0 5 10 15];
    end
    ...
    if nargin < 8 || isempty(label_clrs)
        label_clrs = {[1.00 0.85 0.30], [0.35 0.92 0.55], [1.00 0.48 0.42]};
    end
    if nargin < 9 || isempty(out_base)
        out_base = 'thumb_ndob_animation';
    end
    if nargin < 10 || isempty(title_str)
        title_str = 'Robotic Thumb — NDOB+SMC';
    end
    ...
    TDH_mNm = TAU_DH_Nm * 1000;
    TD_disp  = TD_mNm;
    TDH_disp = TDH_mNm;
    for ji = 1:3
        TD_disp(:,ji)  = movmean(TD_mNm(:,ji),  101);
        TDH_disp(:,ji) = movmean(TDH_mNm(:,ji), 101);
    end
    D_max = max([max(abs(TD_disp(:))), max(abs(TDH_disp(:)))]) * 1.3 + 0.01;
    ...
    h_dhline = gobjects(3,1);
    for ji = 1:3
        h_dline(ji)  = plot(axD, NaN, NaN, '-', ...
                            'Color', dist_clr{ji}, 'LineWidth', 1.8);
        h_dhline(ji) = plot(axD, NaN, NaN, '--', ...
                            'Color', min(dist_clr{ji}*1.05+0.10,[1 1 1]), ...
                            'LineWidth', 1.4);
    end
    h_tcur = xline(axD, t_lo, '-', 'Color', [0.90 0.92 1.00], ...
                   'LineWidth', 1.5, 'Alpha', 0.7);

    legend(axD, [h_dline; h_dhline(1)], ...
           {'\tau_{d1}','\tau_{d2}','\tau_{d3}','hat\tau_d (NDOB)'}, ...
    ...
    % No pre-drawn grey actual trace — incremental cyan 'Tip path' only.
    % Start / End markers
    ...
    text(ax, xl(1)+2, yl(1)+4, ...
         'NDOB+SMC  | \Lambda=12I  K_{new}=diag(2,2,0.3) mNm  L=50I  \delta=0.01 rad', ...
    ...
    try
        ...
        warning('animate_thumb_2d_ndob: VideoWriter failed (%s).', ME.message);
    end
    ...
    dist_dh = {nan(1,nf), nan(1,nf), nan(1,nf)};
    ...
    for k = frames
        ...
        % Right panel: growing disturbance + estimate history + cursor
        dist_t(fi) = tk;
        tdh_k = TDH_disp(k,:);
        for ji = 1:3
            dist_d{ji}(fi)  = td_k(ji);
            dist_dh{ji}(fi) = tdh_k(ji);
            set(h_dline(ji),  'XData', dist_t(1:fi), 'YData', dist_d{ji}(1:fi));
            set(h_dhline(ji), 'XData', dist_t(1:fi), 'YData', dist_dh{ji}(1:fi));
        end
        h_tcur.Value = tk;
