# Disturbance-Observer-Augmented Sliding-Mode Control of a 3-DOF Robotic Thumb — Complete Documentation

---

## Role of the Simulink Reference Model

The reported simulation results in this project are generated by the MATLAB runners,
not by Simulink.

The file `model_thumb_smc_mb.slx` is included as a reference block-diagram model, and
`build_thumb_smc_model.m` can regenerate it through Simulink's programmatic API
(`new_system`, `add_block`, `add_line`). Its role is documentary: it mirrors the
structure of the original coursework model `assignment/model_SMC_mb.slx` in a form that
is easier to inspect visually.

By contrast, `run_thumb_smc_mb.m`, `run_thumb_extra_tests.m`, and `run_thumb_ndob.m`
do not call `sim()`, open the model, or execute the Simulink graph at runtime. State
propagation, dynamics evaluation, and control updates are all carried out in MATLAB code
with fixed-step RK4 integration.

The same distinction applies to the original models in the assignment folder:
`model_SMC_mb.slx`, `model_SMC_mf.slx`, `model_underactuated_NDOB.slx`, and
`model_underactuated_PFLC.slx` provide architectural context, but the numerical results
reported here come from the pure-MATLAB implementation.
